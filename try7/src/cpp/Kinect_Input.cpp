...
{
	(*image).GetAlternativeViewPointCap().SetViewPoint(mImageGenerator);
...
#endif
#if 0
PointCloud<pcl::PointXYZRGB>::Ptr Kinect_Input::pointcloud_generate(DepthGenerator* depth, DepthMetaData* depthmeta, const XnDepthPixel* depthpixel, const XnRGB24Pixel* imgpixel)
{
	int serialy=0;
	int serial=0; 
